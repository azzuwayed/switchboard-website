...
- **Local-first.** No cloud account. No background HTTP server. The only data it sends is an anonymous, aggregate usage count (weekly-active estimate + app version) over the update check — no identifiers, no IP stored, opt out in Settings → Privacy. Your config and logs live in `~/Library/Application Support/`.
